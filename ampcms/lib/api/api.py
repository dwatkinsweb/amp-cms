...
def build_url(site, module_name, page_name, pagelet_urls=None, active=None):
    try:
        if active is not None:
            page = Page.objects.get(name=page_name, module__name=module_name, module__site=site, active=True)
        else:
