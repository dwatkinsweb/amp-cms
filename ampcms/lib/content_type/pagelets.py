...
class MenuPagelet(BasePagelet):
    ...
    def _build_css(self):
        ...
        if site.skin is not None and os.path.exists('%scss/%s/%s' % (settings.STATIC_ROOT, site.skin, _css)):
            self._css.append('%scss/%s/%s' % (settings.STATIC_URL, site.skin, _css))
        return self._css
...
class ApplicationPagelet(BasePagelet):
    ...
    def _build_content(self, process_url=None):
        ...
        try:
            ...
            log.info('Pagelet resolving - url: %s, urlconf: %s' % (self.process_url, application.urlconf), extra={'request':self.request})
            ...
            if isinstance(response, AmpCmsTemplateResponse):
                pagelet_response = response.rendered_content
            elif hasattr(response, 'render'):
                pagelet_response = response.render().content
            elif hasattr(response, 'content'):
                pagelet_response = response.content
            else:
            ...
            set_urlconf(None)
            log.error('Permission Denied loading application pagelet: %s' % e, exc_info=True, extra={'request':self.request})
            kwargs['exception'] = e
            self.title = settings.AMPCMS_403_TITLE
            error_view, kwargs = resolver.resolve403()
            error_response = error_view(self.request, **kwargs)
        except Exception, e:
            set_urlconf(None)
            log.error('Exception loading application pagelet: %s' % e, exc_info=True, extra={'request':self.request})
            kwargs['exception'] = e
            if settings.DEBUG:
                from ampcms.lib.views import debug
                error_response = debug.technical_500_response(self.request, *sys.exc_info())
                if self.request.is_ajax():
                    error_response.content = '<pre>'+error_response.content+'</pre>'
            else:
        ...
        if self.request.is_ajax():
            ...
            return HttpFixedResponse(error_response.content)
