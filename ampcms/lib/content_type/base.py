...
class BaseContentType(object):
    ...
    def css(self):
        ...
        if self._css is None:
            ...
            for _css in css_list_unique:
                if settings.STATIC_URL in _css or 'HTTP://' in _css:
                    self._css.append('@import url("%s");' % _css)
                elif site.skin is not None:
                    skin_css = finders.find(os.path.join('css', site.skin, _css))
                    if skin_css is not None:
                        self._css.append('@import url("%scss/%s/%s");' % (settings.STATIC_URL, site.skin, _css))
                    else:
                        self._css.append('@import url("%scss/%s");' % (settings.STATIC_URL, _css))
                else:
                    self._css.append('@import url("%scss/%s");' % (settings.STATIC_URL, _css))
            self._css = '\n'.join(self._css)
        return self._css
    ...
    def html(self, template=None):
        return self._to_stream(template).render()

    def markup(self, template=None):
        """
